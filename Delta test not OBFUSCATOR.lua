...
-- ── MENÜYÜ SÜRÜKLE (Drag) — PC + MOBİL ─────────────────────
local menuDrag   = false
local menuDragSt = nil   -- başlangıç dokunma pozisyonu
local menuDragPos= nil   -- başlangıç menü pozisyonu

-- TogBtn sürükleme (menü kapalıyken butonu taşı)
local togDrag    = false
local togDragSt  = nil
local togDragPos = nil

-- PC drag: TitleBar üzerinde mouse ile
TitleBar.InputBegan:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1 then
        menuDrag    = true
        menuDragSt  = inp.Position
        menuDragPos = Menu.Position
    end
end)

-- Mobil drag: TitleBar'a dokunma
TitleBar.TouchStarted:Connect(function(touch)
    menuDrag    = true
    menuDragSt  = touch.Position
    menuDragPos = Menu.Position
end)

TitleBar.TouchMoved:Connect(function(touch)
    if not menuDrag or not menuDragSt or not menuDragPos then return end
    local delta = touch.Position - menuDragSt
    Menu.Position = UDim2.new(
        menuDragPos.X.Scale,
        menuDragPos.X.Offset + delta.X,
        menuDragPos.Y.Scale,
        menuDragPos.Y.Offset + delta.Y
    )
end)
...
UserInputService.InputChanged:Connect(function(inp)
    if menuDrag and inp.UserInputType == Enum.UserInputType.MouseMovement then
        if not menuDragSt or not menuDragPos then return end
        local delta = inp.Position - menuDragSt
        Menu.Position = UDim2.new(
            menuDragPos.X.Scale,
            menuDragPos.X.Offset + delta.X,
            menuDragPos.Y.Scale,
    ...
    if togDrag and inp.UserInputType == Enum.UserInputType.MouseMovement then
        if not togDragSt or not togDragPos then return end
        local delta = inp.Position - togDragSt
        TogBtn.Position = UDim2.new(
            togDragPos.X.Scale,
            togDragPos.X.Offset + delta.X,
            togDragPos.Y.Scale,
            togDragPos.Y.Offset + delta.Y
        )
    end
end)
...
TogBtn.InputBegan:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1 then
        togDrag    = true
        togDragSt  = inp.Position
        togDragPos = TogBtn.Position
    end
end)

-- TogBtn sürükleme (Mobil)
TogBtn.TouchStarted:Connect(function(touch)
    togDrag    = true
    togDragSt  = touch.Position
    togDragPos = TogBtn.Position
end)

TogBtn.TouchMoved:Connect(function(touch)
    if not togDrag or not togDragSt or not togDragPos then return end
    local delta = touch.Position - togDragSt
    TogBtn.Position = UDim2.new(
        togDragPos.X.Scale,
        togDragPos.X.Offset + delta.X,
        togDragPos.Y.Scale,
...
TogBtn.TouchEnded:Connect(function()
    togDrag = false
end)
